...
function TaskEditDialog({ task, project, isOpen, onClose, onSave }) {
  ...
  const [formData, setFormData] = useState({
    ...
    deadline: '',
    coverImage: null,
    assignee: '',  // Will store user ID
    assigneeName: '',  // Will store user name for display
    state: 'new',
    priority: 'medium',
  })
  ...
  useEffect(() => {
    if (task) {
      setFormData({
        ...
        assignee: task.assignee_id || task.assignee || '',  // Prefer assignee_id if available
        assigneeName: task.assignee_name || task.assignee || '',  // For display
  ...
  const handleSelectAssignee = (assignee) => {
    setFormData({ 
      ...formData, 
      assignee: assignee.id,  // Store user ID
      assigneeName: assignee.name  // Store name for display
    })
    setShowAssigneeDropdown(false)
  }

  const getSelectedAssignee = () => {
    return availableAssignees.find(a => a.id === formData.assignee)
  }
              ...
              <div className="relative" ref={assigneeDropdownRef}>
                ...
                >
                  <span className="text-gray-900">
                    {formData.assigneeName || formData.assignee || 'Select assignee'}
                  </span>
