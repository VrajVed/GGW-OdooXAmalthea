...
export default function ExpenseModal({ expense, projects, users, onClose, onSave, autoSetUserId = false }) {
  const currentUser = getUser()
  const initialUserId = autoSetUserId && currentUser ? currentUser.id : (expense?.userId || '')
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // Real-time validation for tax amount
      if (name === 'amount' || name === 'tax_amount') {
        ...
        if (taxAmount > amount && amount > 0) {
          setErrors(prev => ({
            ...prev,
            tax_amount: 'Tax amount cannot be greater than the expense amount'
          }))
        } else {
          setErrors(prev => {
            const newErrors = { ...prev }
            delete newErrors.tax_amount
            return newErrors
          })
        }
      }
    ...
    // Clear error for this field (if not tax_amount which is handled above)
    if (errors[name] && name !== 'tax_amount') {
      setErrors(prev => {
        const newErrors = { ...prev }
  ...
  const handleSubmit = async (e, status = 'draft') => {
    ...
    if (!autoSetUserId && !formData.user_id) newErrors.user_id = 'Employee is required'
    if (!formData.category) newErrors.category = 'Category is required'
    if (!formData.amount || parseFloat(formData.amount) <= 0) newErrors.amount = 'Valid amount is required'
    if (!formData.spent_on) newErrors.spent_on = 'Expense date is required'
    
    // Validate tax amount is not greater than amount
    const amount = parseFloat(formData.amount || 0)
    const taxAmount = parseFloat(formData.tax_amount || 0)
    if (taxAmount > amount) {
      newErrors.tax_amount = 'Tax amount cannot be greater than the expense amount'
    }
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(e) => handleSubmit(e, 'draft')} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {/* Employee - Hidden if autoSetUserId is true */}
            {!autoSetUserId ? (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={currentUser?.full_name || 'Current User'}
                  disabled
            ...
            <div>
              ...
              <input
                ...
                step="0.01"
                min="0"
                className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
            ...
            <div>
              ...
              <input
                ...
                min="0"
                className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none ${
                  errors.tax_amount ? 'border-red-300' : 'border-gray-300'
                }`}
              />
              {errors.tax_amount && (
          ...
          <div className="flex items-center">
            ...
            <span className="ml-2 text-xs text-gray-500">
              (If unchecked, this expense will reduce project revenue)
            </span>
          </div>
