...
export default function EmployeeProjectDetailsPage() {
  ...
  const loadProjectDetails = async () => {
    try {
      ...
      // Pass user_id filter for authorization checks
      const userFilters = currentUser?.id ? { user_id: currentUser.id } : {}
      
      const [projectRes, tasksRes] = await Promise.all([
        projectApi.getById(projectId, userFilters),
        taskApi.getAll(projectId, userFilters)
      ])
