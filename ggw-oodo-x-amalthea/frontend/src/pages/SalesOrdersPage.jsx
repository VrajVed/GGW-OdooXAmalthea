...
import { salesOrdersApi, projectApi } from '../lib/api'
...
import SalesOrderBulkActions from '../components/sales-orders/SalesOrderBulkActions'
import SalesOrderDrawer from '../components/sales-orders/SalesOrderDrawer'
...
export default function SalesOrdersPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const ordersResult = await salesOrdersApi.getAll(filters)
      if (ordersResult.success) {
        setSalesOrders(ordersResult.data)
      } else {
        setError(ordersResult.message || 'Failed to load sales orders')
      }

      const statsResult = await salesOrdersApi.getStats(filters)
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedIds(salesOrders.map(o => o.id))
    } else {
      setSelectedIds([])
    }
  }
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={loading}
            >
              <Plus className="inline w-4 h-4 mr-2" /> New Sales Order
            </button>
          </div>
        </div>
          ...
          <>
            {filteredOrders.length === 0 ? (
              <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
                ...
                <p className="text-sm text-gray-500 mb-4">Try adjusting your filters or create a new sales order.</p>
                <button
                  onClick={() => {
                    setEditOrder(null)
                    setShowModal(true)
      ...
      {showDrawer && selectedOrder && (
        <SalesOrderDrawer
          ...
          onUpdate={async () => {
            ...
            if (selectedOrder?.id) {
              try {
                const result = await salesOrdersApi.getById(selectedOrder.id)
                if (result.success) {
