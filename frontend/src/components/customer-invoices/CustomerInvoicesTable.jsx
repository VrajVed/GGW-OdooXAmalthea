...
export default function CustomerInvoicesTable({ customerInvoices, selectedIds, onSelect, onSelectAll, onRowClick }) {
  const allSelected = customerInvoices.length > 0 && selectedIds.length === customerInvoices.length
  const someSelected = selectedIds.length > 0 && selectedIds.length < customerInvoices.length
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {customerInvoices.map((order) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-sm text-gray-900">{formatDate(order.invoiceDate)}</td>
