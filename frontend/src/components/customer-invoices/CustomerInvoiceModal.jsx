...
import { customerInvoicesApi } from '../../lib/api'

export default function CustomerInvoiceModal({ order, projects, onClose, onSave }) {
  ...
  useEffect(() => {
    if (order) {
      setFormData({
        ...
        customer_partner_id: order.customerPartnerId || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (order) {
        await customerInvoicesApi.update(order.id, formData)
      } else {
      ...
      alert('Failed to save customer invoice')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900">
              {order ? 'Edit Customer Invoice' : 'New Customer Invoice'}
            </h2>
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Customer Partner ID</label>
                <input
                  type="text"
                  value={formData.customer_partner_id}
                  onChange={(e) => setFormData(prev => ({ ...prev, customer_partner_id: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                  placeholder="Enter customer partner ID"
                  required
                />
              </div>
