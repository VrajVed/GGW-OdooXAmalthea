...
import { X, CheckCircle2, XCircle, FileText, Link2, Edit, Download, AlertTriangle, Clock } from 'lucide-react'
...
export default function ExpenseDrawer({ expense, onClose, onUpdate, onApprove, onReject, onAddToInvoice }) {
  ...
  const canApprove = expenseData?.status === 'submitted' && 
                     expenseData?.projectId && 
                     expenseData?.userId !== currentUser?.id &&
                     (!(expenseData?.amount > 500) || expenseData?.receiptFileId)

  const canReject = expenseData?.status === 'submitted' || expenseData?.status === 'approved'
