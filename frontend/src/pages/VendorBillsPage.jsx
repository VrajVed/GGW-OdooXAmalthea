...
import { vendorBillsApi, projectApi } from '../lib/api'
import VendorBillsTable from '../components/vendor-bills/VendorBillsTable.jsx'
import VendorBillFilters from '../components/vendor-bills/VendorBillFilters.jsx'
import VendorBillBulkActions from '../components/vendor-bills/VendorBillBulkActions.jsx'
import VendorBillDrawer from '../components/vendor-bills/VendorBillDrawer.jsx'
import VendorBillModal from '../components/vendor-bills/VendorBillModal.jsx'
...
export default function VendorBillsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const ordersResult = await vendorBillsApi.getAll(filters)
      if (ordersResult.success) {
        setVendorBills(ordersResult.data)
      } else {
        setError(ordersResult.message || 'Failed to load vendor bills')
      }

      const statsResult = await vendorBillsApi.getStats(filters)
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedIds(vendorBills.map(o => o.id))
    } else {
      setSelectedIds([])
    }
  }
  ...
  const filteredOrders = vendorBills.filter(order => {
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={loading}
            >
              <Plus className="inline w-4 h-4 mr-2" /> New Vendor Bill
            </button>
          </div>
        </div>
          ...
          <>
            {filteredOrders.length === 0 ? (
              <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
                ...
                <p className="text-sm text-gray-500 mb-4">Try adjusting your filters or create a new vendor bill.</p>
                <button
                  onClick={() => {
                    setEditOrder(null)
                    setShowModal(true)
      ...
      {showDrawer && selectedOrder && (
        <VendorBillDrawer
          ...
          onUpdate={async () => {
            ...
            if (selectedOrder?.id) {
              try {
                const result = await vendorBillsApi.getById(selectedOrder.id)
                if (result.success) {
