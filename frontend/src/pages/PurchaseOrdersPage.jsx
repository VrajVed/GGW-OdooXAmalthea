...
import { purchaseOrdersApi, projectApi } from '../lib/api'
import PurchaseOrdersTable from '../components/purchase-orders/PurchaseOrdersTable.jsx'
import PurchaseOrderFilters from '../components/purchase-orders/PurchaseOrderFilters.jsx'
import PurchaseOrderBulkActions from '../components/purchase-orders/PurchaseOrderBulkActions.jsx'
import PurchaseOrderDrawer from '../components/purchase-orders/PurchaseOrderDrawer.jsx'
import PurchaseOrderModal from '../components/purchase-orders/PurchaseOrderModal.jsx'
...
export default function PurchaseOrdersPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const ordersResult = await purchaseOrdersApi.getAll(filters)
      if (ordersResult.success) {
        setPurchaseOrders(ordersResult.data)
      } else {
        setError(ordersResult.message || 'Failed to load purchase orders')
      }

      const statsResult = await purchaseOrdersApi.getStats(filters)
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedIds(purchaseOrders.map(o => o.id))
    } else {
      setSelectedIds([])
    }
  }
      ...
      <header className="bg-white border-b border-gray-200 px-8 py-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={loading}
            >
              <Plus className="inline w-4 h-4 mr-2" /> New Purchase Order
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-4">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="text-2xl font-semibold text-gray-900">{stats.confirmedCount}</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-colors ${
              JSON.stringify(filters.status) === JSON.stringify(['confirmed'])
                ? 'text-white'
                : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
            }`}
            style={JSON.stringify(filters.status) === JSON.stringify(['confirmed']) ? { backgroundColor: '#714b67' } : {}}
          >
            Confirmed
          </button>
          <button
      ...
      {showDrawer && selectedOrder && (
        <PurchaseOrderDrawer
          ...
          onUpdate={async () => {
            ...
            if (selectedOrder?.id) {
              try {
                const result = await purchaseOrdersApi.getById(selectedOrder.id)
                if (result.success) {
